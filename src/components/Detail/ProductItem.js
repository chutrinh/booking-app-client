...
function ProductItem() {
  ...
  useEffect(() => {
    axios("https://booking-app-agfh.onrender.com", {
      headers: {
      ...
      .then((response) => {
        if (response.data.status === 200) {
          ...
          return new Promise((resolve, reject) => {
            axios(
              "https://booking-app-agfh.onrender.com/detai-product/" +
                params.prductId,
              {
                headers: {
                  "Content-Type": "application/json",
  ...
  const handleAddToCart = () => {
    if (amount > 0) {
      if (window.confirm("bạn có muốn thêm sản phẩm này vào giỏ hàng ?")) {
        axios("https://booking-app-agfh.onrender.com/add-to-cart", {
          headers: {
            "Content-Type": "application/json",
          },
          withCredentials: true,
    ...
    if (imgs.current) {
      ...
      childrenArray.forEach((item) => {
        ...
        item.addEventListener("mouseleave", () => {
          imgMain.current.src =
            "https://booking-app-agfh.onrender.com/" + filterData.img1;
        });
      });
    }
  }, [filterData]);
            ...
            <div className={classes["img"]}>
              <div ref={imgs} className={classes["img-extra"]}>
                <img
                  id={1}
                  src={
                    "https://booking-app-agfh.onrender.com/" + filterData.img2
                  }
                />
                <img
                  id={2}
                  src={
                    "https://booking-app-agfh.onrender.com/" + filterData.img3
                  }
                />
                <img
                  id={3}
                  src={
                    "https://booking-app-agfh.onrender.com/" + filterData.img4
                  }
                />
              </div>
              <img
                ref={imgMain}
                src={"https://booking-app-agfh.onrender.com/" + filterData.img1}
              />
            </div>
          ...
          filterRelated.map((item) => {
            return (
              <li key={item._id} className={classes["product-item"]}>
                <img
                  src={"https://booking-app-agfh.onrender.com/" + item.img1}
                />
                <p>{item.name}</p>
